...
export default factories.createCoreController(
  ...
  ({ strapi }) => ({
    ...
    async getVenuePlayerRecords(ctx) {
      const { venueSlug } = ctx.params;
      const { statKey, season } = ctx.query as { statKey: string; season?: string };
      const service = strapi.service("api::venue.venue");
      const data = await service.findVenuePlayerRecords(venueSlug, statKey, season);
      ctx.body = data;
    },
    ...
    async getVenueTeamRecords(ctx) {
      ...
      const { statKey, season } = ctx.query as { statKey: string; season?: string };
      const service = strapi.service("api::venue.venue");
      const data = await service.findVenueTeamRecords(venueSlug, statKey, season);
      ctx.body = data;
    },
  }),
);
