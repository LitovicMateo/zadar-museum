...
LEFT JOIN (
  ...
  FROM public.zadar_coach_record
  ORDER BY coach_id
) total ON c.coach_id = total.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_coach_record_home
  ORDER BY coach_id
) home ON c.coach_id = home.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_coach_record_away
  ORDER BY coach_id
) away ON c.coach_id = away.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_coach_record_neutral
  ORDER BY coach_id
) neutral ON c.coach_id = neutral.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_head_coach_record
  ORDER BY coach_id
) head ON c.coach_id = head.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_head_coach_record_home
  ORDER BY coach_id
) head_home ON c.coach_id = head_home.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_head_coach_record_away
  ORDER BY coach_id
) head_away ON c.coach_id = head_away.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_head_coach_record_neutral
  ORDER BY coach_id
) head_neutral ON c.coach_id = head_neutral.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_assistant_coach_record
  ORDER BY coach_id
) assistant ON c.coach_id = assistant.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_assistant_coach_record_home
  ORDER BY coach_id
) assistant_home ON c.coach_id = assistant_home.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_assistant_coach_record_away
  ORDER BY coach_id
) assistant_away ON c.coach_id = assistant_away.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id) *
  FROM public.zadar_assistant_coach_record_neutral
  ORDER BY coach_id
