CREATE MATERIALIZED VIEW public.opponent_coach_league_record_full AS

WITH coaches AS (
  -- Ensure exactly one row per coach_id. some source tables may contain
  -- different name variants for the same coach_id which caused duplicate
  -- rows when using UNION across all three sources. Aggregate names
  -- and pick a stable value (MIN) so joins produce a single row per coach.
  SELECT
    coach_id,
...
LEFT JOIN (
  ...
  FROM public.opponent_coach_league_record
  ORDER BY coach_id, league_id
) total
  ON c.coach_id = total.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_coach_league_record_home
  ORDER BY coach_id, league_id
) home
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_coach_league_record_away
  ORDER BY coach_id, league_id
) away
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_coach_league_record_neutral
  ORDER BY coach_id, league_id
) neutral
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_head_coach_league_record
  ORDER BY coach_id, league_id
) head
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_head_coach_league_record_home
  ORDER BY coach_id, league_id
) head_home
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_head_coach_league_record_away
  ORDER BY coach_id, league_id
) head_away
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_head_coach_league_record_neutral
  ORDER BY coach_id, league_id
) head_neutral
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_assistant_coach_league_record
  ORDER BY coach_id, league_id
) assistant
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_assistant_coach_league_record_home
  ORDER BY coach_id, league_id
) assistant_home
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_assistant_coach_league_record_away
  ORDER BY coach_id, league_id
) assistant_away
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, league_id) *
  FROM public.opponent_assistant_coach_league_record_neutral
  ORDER BY coach_id, league_id
) assistant_neutral
