CREATE MATERIALIZED VIEW public.opponent_coach_season_record_full AS

WITH coaches AS (
    SELECT DISTINCT coach_id, first_name, last_name, season FROM public.opponent_coach_season_record
    UNION
    SELECT DISTINCT coach_id, first_name, last_name, season FROM public.opponent_head_coach_season_record
    UNION
    SELECT DISTINCT coach_id, first_name, last_name, season FROM public.opponent_assistant_coach_season_record
)
SELECT
    c.coach_id,
    c.first_name,
...
FROM coaches c
LEFT JOIN public.opponent_coach_season_record total USING (coach_id)
LEFT JOIN public.opponent_coach_season_record_home home USING (coach_id)
LEFT JOIN public.opponent_coach_season_record_away away USING (coach_id)
LEFT JOIN public.opponent_head_coach_season_record head USING (coach_id)
LEFT JOIN public.opponent_head_coach_season_record_home head_home USING (coach_id)
LEFT JOIN public.opponent_head_coach_season_record_away head_away USING (coach_id)
LEFT JOIN public.opponent_assistant_coach_season_record assistant USING (coach_id)
LEFT JOIN public.opponent_assistant_coach_season_record_home assistant_home USING (coach_id)
LEFT JOIN public.opponent_assistant_coach_season_record_away assistant_away USING (coach_id)
