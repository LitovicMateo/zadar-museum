...
WITH coaches AS (
  ...
  FROM (
    SELECT coach_id, first_name, last_name FROM public.opponent_coach_season_record
    UNION ALL
    SELECT coach_id, first_name, last_name FROM public.opponent_head_coach_season_record
    UNION ALL
    SELECT coach_id, first_name, last_name FROM public.opponent_assistant_coach_season_record
  ) t
  GROUP BY coach_id
)
SELECT
...
LEFT JOIN (
  ...
  FROM public.opponent_coach_season_league_record
  ORDER BY coach_id, season, league_id
) total
  ON c.coach_id = total.coach_id
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_coach_season_league_record_home
  ORDER BY coach_id, season, league_id
) home
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_coach_season_league_record_away
  ORDER BY coach_id, season, league_id
) away
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_coach_season_league_record_neutral
  ORDER BY coach_id, season, league_id
) neutral
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_head_coach_season_league_record
  ORDER BY coach_id, season, league_id
) head
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_head_coach_season_league_record_home
  ORDER BY coach_id, season, league_id
) head_home
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_head_coach_season_league_record_away
  ORDER BY coach_id, season, league_id
) head_away
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_head_coach_season_league_record_neutral
  ORDER BY coach_id, season, league_id
) head_neutral
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_assistant_coach_season_league_record
  ORDER BY coach_id, season, league_id
) assistant
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_assistant_coach_season_league_record_home
  ORDER BY coach_id, season, league_id
) assistant_home
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_assistant_coach_season_league_record_away
  ORDER BY coach_id, season, league_id
) assistant_away
...
LEFT JOIN (
  SELECT DISTINCT ON (coach_id, season, league_id) *
  FROM public.opponent_assistant_coach_season_league_record_neutral
  ORDER BY coach_id, season, league_id
) assistant_neutral
