CREATE MATERIALIZED VIEW public.zadar_player_league_record_full AS
SELECT
    total.player_id,
    total.first_name,
    total.last_name,
    ...
    jsonb_build_object(
        ...
        'neutral', jsonb_build_object(
            ...
            'league_id', avg_neutral.league_id,
            'league_slug', avg_neutral.league_slug,
...
FROM (
  ...
  FROM public.zadar_player_total_all_time_league
  ORDER BY player_id, league_id
) total
LEFT JOIN (
  SELECT DISTINCT ON (player_id, league_id) *
  FROM public.zadar_player_total_all_time_league_home
  ORDER BY player_id, league_id
) home
    ON total.player_id = home.player_id
    AND total.league_id = home.league_id
LEFT JOIN (
  SELECT DISTINCT ON (player_id, league_id) *
  FROM public.zadar_player_total_all_time_league_away
  ORDER BY player_id, league_id
) away
    ON total.player_id = away.player_id
    AND total.league_id = away.league_id
LEFT JOIN (
  SELECT DISTINCT ON (player_id, league_id) *
  FROM public.zadar_player_total_all_time_league_neutral
  ORDER BY player_id, league_id
) neutral
    ON total.player_id = neutral.player_id
    AND total.league_id = neutral.league_id
LEFT JOIN (
  SELECT DISTINCT ON (player_id, league_id) *
  FROM public.zadar_player_average_all_time_league
  ORDER BY player_id, league_id
) avg_total
    ON total.player_id = avg_total.player_id
    AND total.league_id = avg_total.league_id
LEFT JOIN (
  SELECT DISTINCT ON (player_id, league_id) *
  FROM public.zadar_player_average_all_time_league_home
  ORDER BY player_id, league_id
) avg_home
    ON total.player_id = avg_home.player_id
    AND total.league_id = avg_home.league_id
LEFT JOIN (
  SELECT DISTINCT ON (player_id, league_id) *
  FROM public.zadar_player_average_all_time_league_away
  ORDER BY player_id, league_id
) avg_away
    ON total.player_id = avg_away.player_id
    AND total.league_id = avg_away.league_id
LEFT JOIN (
  SELECT DISTINCT ON (player_id, league_id) *
  FROM public.zadar_player_average_all_time_league_neutral
  ORDER BY player_id, league_id
) avg_neutral
    ON total.player_id = avg_neutral.player_id
    AND total.league_id = avg_neutral.league_id;
