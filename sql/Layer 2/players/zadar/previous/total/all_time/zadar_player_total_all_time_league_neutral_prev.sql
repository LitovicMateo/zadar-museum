 CREATE MATERIALIZED VIEW public.zadar_player_total_all_time_league_neutral_prev AS
 ...
 SELECT b.player_id,
    ...
    rank() OVER (ORDER BY (sum(b.minutes + (b.seconds / 60.0))) DESC NULLS LAST) AS minutes_rank,
    ...
    rank() OVER (PARTITION BY b.league_id ORDER BY (case when sum(b.field_goals_attempted) = 0 then 0 else round(sum(b.field_goals_made)::numeric /sum(b.field_goals_attempted) * 100, 1) end) DESC NULLS LAST) AS field_goal_percentage_rank,
    ...
    rank() OVER (PARTITION BY b.league_id ORDER BY (case when sum(b.three_pointers_attempted) = 0 then 0 else round(sum(b.three_pointers_made)::numeric /sum(b.three_pointers_attempted) * 100, 1) end) DESC NULLS LAST) AS three_point_percentage_rank,
    ...
    rank() OVER (PARTITION BY b.league_id ORDER BY (case when sum(b.free_throws_attempted) = 0 then 0 else round(sum(b.free_throws_made)::numeric /sum(b.free_throws_attempted) * 100, 1) end) DESC NULLS LAST) AS free_throw_percentage_rank,
