CREATE MATERIALIZED VIEW public.opponent_player_season_average_all_time_neutral AS
 ...
 SELECT b.player_id,
    ...
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.points)) DESC NULLS LAST) AS points_rank,

    round(avg(b.assists), 1) AS assists,
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.assists)) DESC NULLS LAST) AS assists_rank,

    round(avg(b.offensive_rebounds), 1) AS off_rebounds,
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.offensive_rebounds)) DESC NULLS LAST) AS off_rebounds_rank,

    round(avg(b.defensive_rebounds), 1) AS def_rebounds,
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.defensive_rebounds)) DESC NULLS LAST) AS def_rebounds_rank,

    round(avg(b.rebounds), 1) AS rebounds,
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.rebounds)) DESC NULLS LAST) AS rebounds_rank,

    round(avg(b.steals), 1) AS steals,
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.steals)) DESC NULLS LAST) AS steals_rank,

    round(avg(b.blocks), 1) AS blocks,
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.blocks)) DESC NULLS LAST) AS blocks_rank,

    round(avg(b.field_goals_made), 1) AS field_goals_made,
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.field_goals_made)) DESC NULLS LAST) AS field_goals_made_rank,

    round(avg(b.field_goals_attempted), 1) AS field_goals_attempted,
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.field_goals_attempted)) DESC NULLS LAST) AS field_goals_attempted_rank,
    ...
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.three_pointers_attempted)) DESC NULLS LAST) AS three_pointers_attempted_rank,
    ...
    rank() OVER (PARTITION BY b.season ORDER BY (avg(b.free_throws_attempted)) DESC NULLS LAST) AS free_throws_attempted_rank,
  ...
  WHERE 
    ...
    b.status::text <> 'dnp-cd'::text AND b.is_home_team = 'neutral' AND
    b.is_nulled = false
