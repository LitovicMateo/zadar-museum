 CREATE MATERIALIZED VIEW public.opponent_player_season_total_all_time_league_neutral_prev AS
 ...
 SELECT b.player_id,
    ...
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(CASE WHEN b.status::text = 'starter'::text THEN 1 ELSE 0 END)) DESC NULLS LAST) AS games_started_rank,

    sum(b.minutes + (b.seconds / 60.0)) AS minutes,
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.minutes + (b.seconds / 60.0))) DESC NULLS LAST) AS minutes_rank,

    sum(b.points) AS points,
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.points)) DESC NULLS LAST) AS points_rank,

    sum(b.assists) AS assists,
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.assists)) DESC NULLS LAST) AS assists_rank,

    sum(b.offensive_rebounds) AS off_rebounds,
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.offensive_rebounds)) DESC NULLS LAST) AS off_rebounds_rank,

    sum(b.defensive_rebounds) AS def_rebounds,
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.defensive_rebounds)) DESC NULLS LAST) AS def_rebounds_rank,

    sum(b.rebounds) AS rebounds,
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.rebounds)) DESC NULLS LAST) AS rebounds_rank,

    sum(b.steals) AS steals,
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.steals)) DESC NULLS LAST) AS steals_rank,

    sum(b.blocks) AS blocks,
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.blocks)) DESC NULLS LAST) AS blocks_rank,

    sum(b.field_goals_made) AS field_goals_made,
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.field_goals_made)) DESC NULLS LAST) AS field_goals_made_rank,

    sum(b.field_goals_attempted) AS field_goals_attempted,
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.field_goals_attempted)) DESC NULLS LAST) AS field_goals_attempted_rank,
    ...
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.three_pointers_attempted)) DESC NULLS LAST) AS three_pointers_attempted_rank,
    ...
    rank() OVER (PARTITION BY b.league_id, b.season ORDER BY (sum(b.free_throws_attempted)) DESC NULLS LAST) AS free_throws_attempted_rank,
