CREATE MATERIALIZED VIEW public.opponent_player_average_all_time_per_team_per_league_neutral AS
 ...
 SELECT b.player_id,
    ...
    rank() OVER (ORDER BY (case when avg(b.field_goals_attempted) = 0 then 0 else round(avg(b.field_goals_made) / avg(b.field_goals_attempted) * 100, 1) end) DESC NULLS LAST) AS field_goal_percentage_rank,
    ...
    rank() OVER (ORDER BY (case when avg(b.three_pointers_attempted) = 0 then 0 else round(avg(b.three_pointers_made) / avg(b.three_pointers_attempted) * 100, 1) end) DESC NULLS LAST) AS three_point_percentage_rank,
    ...
    rank() OVER (ORDER BY (case when avg(b.free_throws_attempted) = 0 then 0 else round(avg(b.free_throws_made) / avg(b.free_throws_attempted) * 100, 1) end) DESC NULLS LAST) AS free_throw_percentage_rank,
