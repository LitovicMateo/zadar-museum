CREATE MATERIALIZED VIEW public.referee_season_league_stats_away AS
...
WITH kk AS (
         ...
         SELECT g.id AS game_id,
           ...
           FROM games g
			 ...
			 JOIN games_competition_lnk gclnk ON gclnk.game_id = g.id
			 JOIN competitions c ON c.id = gclnk.competition_id
            ...
            WHERE 
                g.away_team_name = 'KK Zadar' AND
                g.is_nulled = FALSE AND
                g.forfeited = FALSE
            GROUP BY g.id, mr.id, sr.id, tr.id, ht.id, at.id, ht.name, at.name, g.season, c.document_id, c.name, c.slug, g.league_name, g.league_short_name
        ), ref_game_union AS (
         SELECT game_scores.main_ref_id AS ref_id,
            game_scores.game_id,
			game_scores.season,
			game_scores.league_id
           FROM game_scores
 ...
 SELECT 
   ...
   FROM ref_game_union u
     ...
     CROSS JOIN kk
  GROUP BY r.id, r.document_id, r.first_name, r.last_name, kk.kk_id, u.season, gs.league_id, gs.league_slug
  ORDER BY (count(DISTINCT
        CASE
            WHEN gs.home_team_id = kk.kk_id OR gs.away_team_id = kk.kk_id THEN u.game_id
            ELSE NULL::integer
