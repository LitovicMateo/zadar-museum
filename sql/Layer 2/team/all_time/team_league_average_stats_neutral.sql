CREATE MATERIALIZED VIEW public.team_league_average_stats_neutral AS
...
ranks AS (
    SELECT
        league_id,
        team_id,
        RANK() OVER (PARTITION BY league_id ORDER BY games DESC)          AS games_rank,
        RANK() OVER (PARTITION BY league_id ORDER BY wins DESC)           AS wins_rank,
        RANK() OVER (PARTITION BY league_id ORDER BY losses DESC)         AS losses_rank,
        RANK() OVER (PARTITION BY league_id ORDER BY points_scored DESC)  AS points_scored_rank,
        RANK() OVER (PARTITION BY league_id ORDER BY points_diff DESC)    AS points_diff_rank,
        RANK() OVER (PARTITION BY league_id ORDER BY points_received DESC)AS points_received_rank,
        RANK() OVER (PARTITION BY league_id ORDER BY attendance DESC)     AS attendance_rank
    FROM agg_no_zadar
)
