CREATE MATERIALIZED VIEW public.zadar_player_record_away AS
...
SELECT
    ...
    rank() OVER (ORDER BY (field_goals_percentage) DESC NULLS LAST) AS field_goals_percentage_rank,
    ...
    rank() OVER (ORDER BY (free_throws_percentage) DESC NULLS LAST) AS free_throws_percentage_rank,
