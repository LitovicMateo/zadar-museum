...
/** Format a nullable counting stat (points, rebounds, assists, ...). */
export const displayStat = (v: number | null): string => (v === null ? '-' : String(v));
 ...
 * Returns '-' if made is null. If attempted is missing, shows only the made value
 * (no attempts recorded), rather than a "made/-" line.
 */
export const formatMakeAttempt = (made: number | null, attempted: number | null): string => {
	...
	if (attMissing) return String(made);
