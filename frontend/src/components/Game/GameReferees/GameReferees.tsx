...
const GameReferees: React.FC = () => {
	...
	return (
		<Card className="gap-0 overflow-hidden border-border py-0 shadow-xs">
			<CardHeader className="flex items-center gap-2 bg-court px-3 py-1.5">
				...
				<CardTitle className="text-[10px] font-mono font-semibold uppercase tracking-[0.12em] text-court-foreground/90">
					Officials
				</CardTitle>
			</CardHeader>
			<CardContent className="flex flex-wrap gap-2 px-3 py-3">
				{refs?.map((ref) => (
					<Link
						...
						className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 text-sm font-medium text-foreground transition-colors hover:border-record hover:text-record"
					>
						<span className="size-1.5 rounded-full bg-record" aria-hidden />
...
const WhistleSVG = () => {
	return (
		<svg className="h-4 w-4" fill="currentColor" version="1.1" x="0px" y="0px" viewBox="0 0 122.88 81.03">
			<g>
