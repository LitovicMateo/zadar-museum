import React from 'react';
import { Link } from 'react-router-dom';
import Flag from 'react-world-flags';

import { APP_ROUTES } from '@/constants/Routes';
import { RefereeDirectoryEntry } from '@/types/api/Referee';
import { getImageUrl } from '@/utils/GetImageUrl';

import styles from './RefereeCard.module.css';

interface RefereeCardProps {
	referee: RefereeDirectoryEntry;
}

const RefereeCard: React.FC<RefereeCardProps> = ({ referee }) => {
	const imageUrl = referee.image?.url ? getImageUrl(referee.image.url) : '';
	const hasImage = !!imageUrl && !imageUrl.includes('undefined');

	return (
		<Link to={APP_ROUTES.referee(referee.document_id)} className={styles.cardLink}>
			<div className={styles.card}>
				<div className={styles.imageWrapper}>
					{hasImage ? (
						<img src={imageUrl} alt={referee.name} className={styles.image} loading="lazy" />
					) : (
						<div className={styles.placeholder}>
							<WhistleSVG />
						</div>
					)}

					<div className={styles.gradient} />

					{referee.nation && (
						<Flag className={styles.flagBadge} code={referee.nation} aria-label={referee.nation} />
					)}

					<div className={styles.nameplate}>
						<p className={styles.name}>
							<span className={styles.firstName}>{referee.first_name} </span>
							<span className={styles.lastName}>{referee.last_name}</span>
						</p>
					</div>
				</div>

				<div className={styles.statsStrip}>
					{(
						[
							{ label: 'G', value: referee.games },
							{ label: 'F+', value: referee.fouls_for },
							{ label: 'F-', value: referee.fouls_against },
							{ label: 'FD', value: referee.foul_difference_best },
						] as const
					).map(({ label, value }) => (
						<div key={label} className={styles.stat}>
							<span className={styles.statLabel}>{label}</span>
							<span className={styles.statValue}>{value ?? '–'}</span>
						</div>
					))}
				</div>
			</div>
		</Link>
	);
};

export default RefereeCard;

const WhistleSVG = () => (
	<svg
		width="52"
		height="52"
		fill="var(--muted-foreground)"
		viewBox="0 0 122.88 81.03"
		opacity={0.4}
	>
		<path d="M112.28,29.7c-0.09-0.06-0.18-0.12-0.26-0.2l-2.85-2.59c-0.89-0.62-1.84-1.15-2.86-1.58c-9.45-3.97-18.65,0.75-27.18,5.58 c-0.04,0.02-0.07,0.04-0.11,0.06c-0.46,0.26-0.93,0.53-1.39,0.79c-1.31,0.74-2.6,1.48-3.7,2.08l-48.13,26.4l0.18,11.18l19.34-9.67 c4.92-2.41,9.65-3.9,14.01-3.53c4.6,0.39,8.65,2.76,11.87,8.09l0.01,0.02c1.39,2.05,2.89,3.76,4.48,5.16 c1.6,1.39,3.29,2.47,5.08,3.27c0.59,0.26,1.22,0.51,1.87,0.74c6.58,2.32,13.6,1.72,19.7-1.08c6.12-2.8,11.31-7.8,14.18-14.25 c0.29-0.64,0.56-1.32,0.81-2.04c2.05-5.81,1.94-13.43-0.33-20.05C115.93,34.94,114.35,32.03,112.28,29.7L112.28,29.7z M97.87,10.91 c3.31-3.96,7.77-5.34,11.75-4.84c2.13,0.27,4.13,1.09,5.76,2.34c1.67,1.29,2.95,3.03,3.58,5.12c0.02,0.06,0.03,0.13,0.05,0.19 c1.01,3.6,0.13,8.12-3.97,12.92c2.59,2.81,4.53,6.32,5.84,10.12c2.56,7.45,2.66,16.09,0.32,22.74c-0.27,0.77-0.59,1.56-0.94,2.35 c-3.29,7.38-9.21,13.09-16.21,16.3c-7.02,3.22-15.13,3.91-22.75,1.22c-0.7-0.25-1.43-0.54-2.17-0.87 c-2.15-0.96-4.19-2.25-6.1-3.92c-1.85-1.62-3.59-3.59-5.18-5.94c-0.05-0.06-0.09-0.13-0.13-0.2c-2.45-4.06-5.41-5.84-8.71-6.13 c-3.54-0.3-7.58,1.01-11.88,3.11l-22.1,11.05c-0.77,0.44-1.76,0.35-2.44-0.3L0.62,54.96l0,0C0.23,54.58-0.01,54.05,0,53.47 l0.22-14.35c-0.02-0.52,0.16-1.06,0.55-1.47c0.18-0.2,0.4-0.35,0.62-0.46L41.6,14.46c0.07-0.05,0.15-0.09,0.23-0.13l13.39-7.57 c0.08-0.05,0.16-0.1,0.25-0.14l1.16-0.66c0.07-0.05,0.14-0.09,0.22-0.13c3.65-1.9,7.28-3.45,10.85-4.48 c3.64-1.05,7.24-1.54,10.75-1.29C86.88,0.67,92.42,5.65,97.87,10.91L97.87,10.91z" />
	</svg>
);
