...
const RefereeCard: React.FC<RefereeCardProps> = ({ referee }) => {
	const imageUrl = referee.image?.url ? getImageUrl(referee.image.url) : '';
	const hasImage = !!imageUrl && !imageUrl.includes('undefined');

	return (
		<Link to={APP_ROUTES.referee(referee.document_id)} className={styles.card}>
			<div className={styles.imageWrapper}>
				{hasImage ? (
					<img src={imageUrl} alt={referee.name} className={styles.image} loading="lazy" />
				) : (
					// something to represent a referee - maybe a whistle silhouette
					<WhistleSVG />
				)}
				{referee.nation && (
					<Flag className={styles.flagBadge} code={referee.nation} aria-label={referee.nation} />
				)}
			</div>
			...
			<div className={styles.body}>
				...
				<div className={styles.stats}>
					<div className={styles.stat}>
						...
						<span className={styles.statValue}>{referee.games ?? '-'}</span>
					</div>
					<div className={styles.stat}>
						<span className={styles.statLabel}>F+</span>
						<span className={styles.statValue}>{referee.fouls_for ?? '-'}</span>
					</div>
					<div className={styles.stat}>
						<span className={styles.statLabel}>F-</span>
						<span className={styles.statValue}>{referee.fouls_against ?? '-'}</span>
					</div>
					<div className={styles.stat}>
						<span className={styles.statLabel}>FD</span>
						<span className={styles.statValue}>{referee.foul_difference_best ?? '-'}</span>
					</div>
				</div>
			</div>
		</Link>
...
// SVG for a whistle icon, used as a placeholder when no referee image is available
// style it with a light gray fill and maybe some simple details to make it recognizable as a whistle
const WhistleSVG = () => {
	return (
		<svg
