...
const GameReferees: React.FC = () => {
	...
	const { data: refs, isLoading } = useGameReferees(gameId!);

	if (refs?.length === 0 || isLoading) return null;

	return (
		<div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl shadow-md border-2 border-slate-200 p-6">
			<div className="flex flex-col gap-4">
				<div className="flex items-center gap-3">
					<div className="flex items-center justify-center w-10 h-10 bg-slate-700 rounded-lg shadow-sm">
						<WhistleSVG />
					</div>
					<div>
						<h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">Officials</h3>
						<p className="text-xs text-gray-500">Game Referees</p>
					</div>
				</div>
				...
				<div className="flex flex-wrap gap-2">
					{refs?.map((ref, idx) => (
						<Link
							...
							className="bg-white hover:bg-blue-50 border-2 border-gray-200 hover:border-blue-400 rounded-lg px-4 py-2 transition-all duration-200 hover:shadow-md group"
						>
...
export default GameReferees;

const WhistleSVG = () => {
	return (
		<svg className="w-6 h-6" fill="white" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 122.88 81.03">
			<g>
