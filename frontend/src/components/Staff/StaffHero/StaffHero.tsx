...
const StaffHero: React.FC = React.memo(() => {
	const { staffId } = useParams();
	const { data: staff, isLoading } = useStaffDetails(staffId!);

	if (isLoading || !staff) return <HeroSkeleton />;

	return <HeroBanner staff={staff} />;
});

StaffHero.displayName = 'StaffHero';

const HeroBanner: React.FC<{ staff: StaffDetailsResponse }> = ({ staff }) => {
	const imageUrl = getImageUrl(staff.image?.url);
		...
		<section className="relative overflow-hidden bg-court text-white" aria-label={`${fullName} profile`}>
			...
			<div className="relative mx-auto grid max-w-6xl grid-cols-[auto_1fr] items-start gap-x-4 gap-y-5 px-4 pb-6 pt-7 sm:items-end sm:gap-x-7 sm:px-6 sm:pt-9 md:pb-8">
				...
				<div className="col-start-2 flex flex-wrap items-center gap-2">
					{staff.nationality && (
						<Chip label="Nat">
							<Flag code={staff.nationality} className="h-3.5 w-5 rounded-[2px] object-cover" />
						</Chip>
					)}
					{staff.role && <Chip label="Role">{staff.role}</Chip>}
				</div>
			</div>
		</section>
	);
